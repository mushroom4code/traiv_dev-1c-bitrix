...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		locationsCompletion: function()
		{
			...
			/*для вывода верхнего уведомления uncomment*/
		...
		createBasketItemContent: function(data)
		{
			...
			return BX.create('DIV', {
				...
				children: propsNodes.length ? [
					BX.create('DIV', {props: {className: 'bx-soa-item-title'}, html: titleHtml.replace("Китай", "").replace("Европа", "")}),
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			if (data.PRICE == 0){
				this.basketBlockNode.appendChild(BX.create('DIV', {props: {className: 'checkZeroPrice'}, text: ''}));
			}
			...
			{
				textNode.appendChild(BX.create('STRONG', {props: {className: 'bx-price all'}, html: data.SUM == '0 руб.' ? 'По запросу' : data.SUM}));
				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
					textNode.appendChild(BX.create('BR'));
					textNode.appendChild(BX.create('STRONG', {
...
//						BX.create('BR')
					...
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						label = BX.create('LABEL', {
							children: [
								...
								BX.create('SPAN', {props: {className: 'bx-person-name-span'}, text: currentType.NAME})
		...
		editDeliveryBlock: function(active)
		{
			...
			this.checkPickUpShow();
			/*было true, добавлено 28022023*/
			this.initialized.delivery = false;
			/*end было true, добавлено 28022023*/
		},
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				deliveryNode = BX.create('DIV', {props: {className: 'bx-soa-pp row h-100'}});
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-8 bx-soa-pp-item-container zind'}}),
				deliveryItemNode, k;
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-md-4 bx-soa-pp-desc-container'}}),
			...
			deliveryInfoContainer.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-pp-company h-100'},
					children: [subTitle, label, title, clear, extraServicesNode, infoList]
				})
			);
			if (window.screen.width < 768) {
				// this.deliveryBlockNode.querySelector('input[type=checkbox][name=DELIVERY_ID]:checked').closest()
				deliveryNode.querySelector('.bx-soa-pp-company.bx-selected').append(deliveryInfoContainer)
			} else {
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-delivery', id:'del_address_true'}}),
			...
			if (currentDelivery.ID == 3){
				document.getElementById('del_address_true').remove();
			}
		...
		createDeliveryItem: function(item)
		{
			...
			if (1==1/*item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined'*/)
			{
				/*dkv*/
				if(item.ID === '3') {
					labelNodes.push(
						BX.create('DIV', {
							children: [
								BX.create('DIV', {
									props: {className: 'bx-soa-pp-delivery-new-name'},
			...
			label = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-graf-container d-flex align-items-baseline'
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				if (property.getId() == 33) {
					BX.addClass(propsItemNode, "form-group bx-soa-customer-field needcallopercheck");
				} else {
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			if (disabled)
			{
				...
				{
					values = [];
					inputs = prop.querySelectorAll('input[type=text], input[type=tel]');
					if (inputs.length == 0)
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					var target = e.target || e.srcElement,
						...
						textInputs = targetContainer.querySelectorAll('input[type=text], input[type=tel]'),
		...
		{
			var propsCommentContainer, label, input, div, textDescription, decodeList, decodeListText;
			decodeList = BX.parseJSON(BX.getCookie('order_decode_list'));
			if (decodeList){
				decodeListText = 'Данные добавленные из личного кабинета пользователя:\n\n';
				decodeList.forEach(function(element){
					decodeListText += element.dElement + '\n';
				});
				textDescription = this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : '' + '\n' + decodeListText;
			} else {
				textDescription = this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : '';
			}
			...
			input = BX.create('TEXTAREA', {
				...
				text: /*this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : ''*/textDescription
			});
			div = BX.create('DIV', {
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'btn-blue btn-order-save w100'
								},
		...
		createTotalUnit: function(name, value, params)
		{
			...
			if (params.total){
				...
				var checkZeroPrice = this.basketBlockNode.querySelector('.checkZeroPrice');

				if (checkZeroPrice)
				{

					return BX.create('DIV', {
						props: {className: 'checkZeroPriceText'},
						text: 'Сумма заказа будет сформирована для вас менеджером!',
						children: [
							BX.create('span', {
								text:'Сумма заказа будет сформирована для вас менеджером!'
							})
