...
<div class="row">
   ...
   <!-- <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-6 text-center text-sm-right text-md-right text-lg-right text-xl-right">
    <span class="sale-order-detail-payment-options-methods-info-title-status-bonus">
    <?php 
...
/*    function num_word($value, $words, $show = true)
    {
        ...
        $koef = 1;
        $sumBonus = ceil(($arResult["PRICE"] * $koef)/120);*/
    ?>
    	При оплате заказа Вы получите <?/*= num_word($sumBonus, array('бонус', 'бонуса', 'бонусов'))*/?> на счет!
    	</span>
    </div>-->
			...
			<? if ($arParams['GUEST_MODE'] !== 'Y')
			{
				?>
				<div class="mb-3">
					<a href="/personal/orders/<?/*= htmlspecialcharsbx($arResult["URL_TO_LIST"]) */?>">&larr; <?= Loc::getMessage('SPOD_RETURN_LIST_ORDERS') ?></a>
				</div>
				<?
			}
			?>
				...
				<div class="col sale-order-detail-card">
						...
						<div class="col p-0">
							...
							<div class="row m-0 align-items-center mt-3">
								...
								<div class="col-sm mb-3">
									<span class="sale-order-detail-prop-name"><?= Loc::getMessage('SPOD_ORDER_PRICE')?>:</span>
									<span class="sale-order-detail-prop-value"> <?= $arResult["PRICE_FORMATED"]?></span>
								</div>

								<? if ($arParams['GUEST_MODE'] !== 'Y')
								{
									?>
									<div class="col-sm-auto mb-3 text-right">
										
										<div class="btn-group-blue d-none"><a href="#<?/*=$arResult["URL_TO_COPY"]*/?>" class="btn-404" id="repeat-order"><span><i class="fa fa-refresh"></i> Повторить заказ</span></a></div>

										<!-- <a href="<?=$arResult["URL_TO_COPY"]?>" class="btn btn-primary btn-block btn-sm my-1"><?= Loc::getMessage('SPOD_ORDER_REPEAT') ?></a> -->
									...
									<div class="col-sm-auto mb-3 text-right">
										...
										<div class="btn-group-blue"><a href="#<?/*=$arResult["URL_TO_COPY"]*/?>" class="btn-404" id="repeat-cart"><span><i class="fa fa-refresh"></i> <!-- Копировать корзину --> Повторить заказ</span></a></div>
									</div>
												...
												<div class="col sale-order-detail-payment-options-methods-shipment-list">
													...
													<? if($shipment["DELIVERY_NAME"] <> '')
													{
														...
														<div
															...
															<?= Loc::getMessage('SPOD_ORDER_DELIVERY') ?> : <span class="sale-order-detail-sum-number"><?= htmlspecialcharsbx($shipment["DELIVERY_NAME"]) ?></span>
															</div>
														<?
													}
													?>
					...
					<div class="row sale-order-detail-total-payment">
						<div class="col sale-order-detail-total-payment-container">
							<div class="row">
								<ul class="col-md-2 col sale-order-detail-total-payment-list-left">
								...
								<ul class="col-md-10 col sale-order-detail-total-payment-list-right">
									<?
									if (floatval($arResult["ORDER_WEIGHT"])) { ?>
				...
				if ($arParams['GUEST_MODE'] !== 'Y' && $arResult['LOCK_CHANGE_PAYSYSTEM'] !== 'Y')
			{
				...
				<div class="row mb-3">
					<div class="col">
						<a href="/personal/orders/<?/*= $arResult["URL_TO_LIST"] */?>">&larr; <?= Loc::getMessage('SPOD_RETURN_LIST_ORDERS')?></a>
					</div>
				</div>
				<?
			}
	...
	$javascriptParams = array(
		...
		"repeatUrlcart" => CUtil::JSEscape($this->GetFolder().'/repeat_order1.php'),
	    "repeatUrlorder" => CUtil::JSEscape($this->GetFolder().'/repeat_order.php'),
		"repeatUrlNumber" => $arResult["ACCOUNT_NUMBER"]
	);
