...
$origPage = $APPLICATION->GetCurPage(false);
...
$kombox_str = CKomboxFilter::GetCurPageParam();

$fpos = strpos( $kombox_str,'/filter');
if ($fpos !== false){
    $fcutstring = substr($kombox_str, $fpos);
    $fpos1 = strpos( $fcutstring,'?');
    $res_string = substr($fcutstring, 1, $fpos1);
    $arResult['sUrlPathParams'] = str_replace('index.php?',$res_string, $arResult['sUrlPathParams']);
}


?>

<div class="row g-0 pt-3 pb-3 bx_pagination_bottom">
    <div class="col-12 col-xl-6 col-lg-6 col-md-6 text-left bx_pagination_section_two">
    ...
    <div class="col-12 col-xl-6 col-lg-6 col-md-6 text-right bx_pagination_section_one">
            ...
            <div class="bx_pagination_page"><?
                ...
                else
                {
                    ...
                    if (true === $arResult["bDescPageNumbering"])
                    {
                        ...
                        {
                            ?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>" title="<? echo GetMessage('nav_prev_title'); ?>">&#8592;</a><?
                        }
                        else
                        {
                            ?>&#8592;<?
                        ...
                        {
                            ?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>" title="<? echo GetMessage('nav_next_title'); ?>">&#8594;</a><?
                        }
                        else
                        {
                            ?>&#8594;<?
                    ...
                    else
                    {
                        ?>
                        <li><?
                        if (2 < $arResult["NavPageNomer"])
                        {
                            ?><a href="<?=$arResult['sUrlPathParams']; ?>PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]-1)?>" title="<? echo GetMessage('nav_prev_title'); ?>">&#8592;</a><?
                        }
                        elseif ($arResult["NavPageNomer"] == 2)
                        {
                            ?><a href="<?=$origPage.$res_string ?>" title="<? echo GetMessage('nav_prev_title'); ?>" rel="rhis">&#8592;</a><?
                        }
                        else
                        {
                            ?>&#8592;<?
                        ...
                        while($NavRecordGroup <= $arResult["NavPageCount"])
                        {
                            ...
                            {
                                ?><li><a href="<?=$origPage.$res_string; ?>" title="<? echo $strTitle; ?>"><?=$NavRecordGroup?></a></li><?
                            }
                            else
                            {
