...
<div class="bx-filter island <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "bx-filter-horizontal"?> ">
									...
									<div class="col-xs-10 col-xs-offset-1 bx-ui-slider-track-container">
										<div class="bx-ui-slider-track" id="drag_track_<?=$key?>">
											<?for($i = 0; $i <= $step_num; $i++):
												...
												?>
												<div class="bx-ui-slider-part p<?=$i+1?>"><span><?=($i == 4) ? ceil($prices[$i]) : floor($prices[$i])?></span></div>
											<?endfor;?> 
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					if (
						...
						continue;
                                        ...
                                        // Если стоит ограничение на группу, то скроем фильтры
                                        $arPage = explode("/", $APPLICATION->GetCurPage());
                                        $code = $arPage[3];
                                        if (isset($arResult["HIDE_FILTER"][$code]) && (!in_array($arItem["CODE"], $arResult["HIDE_FILTER"][$code]))) continue;
					?>
					<div class="c-filter__section <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active is-opened<?endif?>">
                                            <span class="bx-filter-container-modef"></span>
                                            <h5 class="c-filter__toggle">
                                                <?=$arItem["NAME"]?>
												...
												<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
                                                                                                    ...
                                                                                                    <input
                                                                                                        class="checkbox__input"
                                                                                                            ...
                                                                                                            onclick="smartFilter.click(this)"
                                                                                                    />
														<span class="checkbox__inner" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
														if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
															?>&nbsp;<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span><?
														endif;?></span>
				...
				?>
						...
						<div class="c-filter__controls">
							...
							<input
								...
								value="Сбросить"
							/>
							<div class="c-filter__tooltip is-visible" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
								<?echo GetMessage("CT_BCSF_FILTER_COUNT", array("#ELEMENT_COUNT#" => '<span id="modef_num">'.intval($arResult["ELEMENT_COUNT"]).'</span>'));?>

								<a class="btn btn--submit btn--small" href="<?echo $arResult["FILTER_URL"]?>" target=""><?echo GetMessage("CT_BCSF_FILTER_SHOW")?></a>
							</div>
						</div>
