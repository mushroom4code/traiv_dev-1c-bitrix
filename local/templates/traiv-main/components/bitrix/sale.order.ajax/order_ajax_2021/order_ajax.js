...
/*для вывода верхнего уведомления uncomment*/
		...
		editTotalBlock: function()
		{
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'test bx-soa-cart-total-button-container' + (!showOrderButton ? ' visible-xs' : '')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
				...
				this.totalInfoBlockNode.appendChild(
						BX.create('DIV', {
							...
							text: 'Обратите внимание: если сумма заказа менее 500 р.,действует розничная наценка 30%',
							children: [
								BX.create('I', {
									props: {
										className: 'icofont icofont-warning-alt'
									}
								}),
