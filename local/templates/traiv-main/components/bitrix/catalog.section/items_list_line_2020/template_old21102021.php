...
?>
    ...
    <ul class="row traiv-catalog-line-default loadmore_wrap">
        ...
        ?>
        <div class="item-list-container"><li class="col x1 x1--t x1--s" id='<?= $strMainID ?>'><div class="new-item-line loadmore_item"><div class="position-list xol p3"><?=$position.'. '?></div><div class="col x1d12 x1d8--m"><div class="new-item-line__image overflow-h"><?$checkUrl = str_replace('https://traiv-komplekt.ru','',$item['CANONICAL_PAGE_URL']);
if ($item['DETAIL_PAGE_URL'] !== $checkUrl) {$item['DETAIL_PAGE_URL'] = $checkUrl;}?><a href="<?= $item['DETAIL_PAGE_URL'] ?>" rel="<?php echo $item["CANONICAL_PAGE_URL"];?>"><?if(!empty($item['DETAIL_PICTURE']['SRC'])):?>
<?$picturl = CFile::ResizeImageGet($item['DETAIL_PICTURE'],array('width'=>35, 'height'=>35), BX_RESIZE_IMAGE_PROPORTIONAL, true);?><img src="<?=$picturl['src']?>" alt="<?=$formatedname?>" id="<?=$item["ID"]?>"/><?else:
                                    ...
                                    if($arElement = $rsElement->Fetch())$pict = $arElement['DETAIL_PICTURE']?$arElement['DETAIL_PICTURE']:$arElement['PICTURE'];
                                    $picturl = CFile::ResizeImageGet($pict,array('width'=>35, 'height'=>35), BX_RESIZE_IMAGE_PROPORTIONAL, true);?><img  src="<?= $picturl['src']?>" alt="<?= $formatedname?>" title="<?=$formatedname?>" id="<?=$item["ID"]?>"/><?endif?></a></div></div><div class="new-item-line__header col x3d10 x3d4--m"><div class="new-item-line__title"><a href="<?= $item['DETAIL_PAGE_URL'] ?>" style="font-size:14px;"><?=$formatedname?></a></div></div><div class="col x1d10 size"><span><?php echo $item["PROPERTIES"]["CML2_ARTICLE"]["VALUE"];?></span></div><div class="col x1d10 size"><span><?=$dlina ? '<div style="display: inline-block">M </div> ' . $diametr.' x '.$dlina : $diametr?></span></div><div class="col x1d12 material"><span><nobr><?=$material?></nobr></span></div><div class="col x1d8 x1d3--m"><span class="new-item-line__price"><span>
<?if ($printPriceValue !== '0 руб.'){echo $printPriceValue;} else {?><a href="#w-form-one-click" class="btn new-item-line-buy opt"><div class="opt-btn-label">Запросить</div></a><?}?></span></span></div><div class="col x1d12 x1d3--m"><?$ymarket = $item["PROPERTIES"]["YMARKET"]["VALUE"];!$ymarket ? $pack = $item["PROPERTIES"]["KRATNOST_UPAKOVKI"]["VALUE"] : $pack = 1;!$pack && $pack = 1;?><input type="number" name='QUANTITY' class="quantity section_list" id="<?= $item["ID"]?>-item-quantity"  size="5" value="<?=$pack?>" step="<?=$pack?>" min="<?=$pack?>"></div><div class="col x1d12 x1d3--m list-cart"><?php if($item['CAN_BUY']) {$item['~ADD_URL'] .= '&QUANTITY=';?><a data-href="<?= str_replace("index.php", "", $item['~ADD_URL']); ?>" class="btn new-item-line-buy" data-ajax-order><img src="/img/ico/cart-30x25.png"><div class="buy-btn-label"><?= $buttonLabel ?></div></a>
                        <?}
