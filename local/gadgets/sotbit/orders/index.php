...
if(Loader::includeModule('sotbit.cabinet') && Loader::includeModule('sale') && $idUser > 0)
{
    $limit = 2;
	...
	$listOrders = new \Sotbit\Cabinet\Shop\Orders();
	...
	foreach ($orders as $order)
	{
        ...
        <div class="order-status">
            <a href="<?= $order->getUrl($arParams['G_ORDERS_PATH_TO_ORDER_DETAIL']) ?>"
               ...
               title="<?= Loc::getMessage('GD_SOTBIT_CABINET_ORDER_ORDER') ?> <?= $order->getId() ?>">
                <div class="order-status-title">
					<?= Loc::getMessage('GD_SOTBIT_CABINET_ORDER_ORDER') ?> <?= $order->getId() ?>
                </div>
            </a>
            <div class="order-status-info">
                <div>
					<?= Loc::getMessage('GD_SOTBIT_CABINET_ORDER_FROM') ?> <?= $order->getDate()->format("d.m.Y") ?>
                </div>
                <div>
                    <span><?= Loc::getMessage('GD_SOTBIT_CABINET_ORDER_SUM') ?>: </span>
