...
$(document).ready(function(){
	let page = $('*').not($(".content").find("*"));
	let img = $('.content img');

	if($('div').hasClass('menu-tr')){

		page.removeClass("iv-dashed").removeAttr("data-intervolga-tips-id");
		img.removeClass("iv-dashed").removeAttr("data-intervolga-tips-id");
/*		var html = document.getElementsByClassName('news-item')[0];
		html.innerHTML = str.innerHTML.replace(/,/gi, '');*/
		img.each(function () {

			if ($(this).parent('.eraser-9000').text() !== ''){

			let title = $(this).parent().parent().find('.news-item__title').text();
			...
			$(this).next('span').remove();
					...
					$(this).parent('.eraser-9000').contents().filter((_, el) => el.nodeType === 3).remove();
			}

		})
		$('.nav__link:nth-child(3)').html("Соответствие<span>DIN - ГОСТ - ISO</span>");
	}
});