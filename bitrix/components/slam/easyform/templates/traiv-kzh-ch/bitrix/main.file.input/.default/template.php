...
<div class="file-input file-extended">
	...
	<?if ($arParams["ALLOW_UPLOAD"] != "N")
	{
		?><div class="webform-field-upload" id="mfi-<?=$arParams['CONTROL_ID']?>-button"><?
			...
			?><span class="btn-file-clip webform-small-button webform-button-upload"><i class="fas fa-paperclip"></i><?//=$inputCaption ?></span><?
        if (!empty($arParams["ALLOW_UPLOAD_EXT"]) || $arParams['MAX_FILE_SIZE'] > 0)
        {
            ...
            ?>
            <div class="webform-field-upload-notice"><?=GetMessage("MFI_NOTICE_5")?></div>
            <div class="webform-field-upload-notice-small"><?=GetMessage("MFI_NOTICE_6")?><?//=str_replace(array("#ext#", "#size#"), array(htmlspecialcharsBx($arParams["ALLOW_UPLOAD_EXT"]), CFile::FormatSize($arParams['MAX_FILE_SIZE'])), $message);?></div><?
        }
