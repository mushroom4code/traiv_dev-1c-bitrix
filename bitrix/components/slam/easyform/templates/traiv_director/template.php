...
if (!empty($arParams["FORM_NAME"])) {
    ?>
    <div class='main-title-text'><?php echo $arParams["FORM_NAME"];?></div>
    <?php 
}
?>
    ...
    <form id="<?=$FORM_ID?>"
        ...
        <div class="row">
            ...
            if(!empty($arResult['FORM_FIELDS'])):
                    ...
                    if($arField['TYPE'] == 'textarea'):?>
                        <div class="col-xs-12">
                            <div class="form-group">
                                ...
                                <? endif; ?>
                                <div>
                                    <textarea class="form-control" id="<?=$arField['ID']?>" rows="5" name="<?=$arField['NAME']?>" <?/*= $arParams["FORMATED_NAME"] ? 'Меня интересует оптовая цена на '.$arParams["FORMATED_NAME"] :  $arField['PLACEHOLDER_STR'];*/?> placeholder="Текст Вашего обращения" <?=$arField['REQ_STR']?> maxlength="600" style="height:70px;"></textarea>
                                </div>
                            </div>
                        </div>
                ...
                <button type="submit" class="btn-blue submit-button submit-big-text w100" data-default="<?=$arParams['FORM_SUBMIT_VALUE']?>" rel="1"><?= ($arParams['FORM_ID'] === 'FORM44') ? 'Оставить заявку' : $arParams['FORM_SUBMIT_VALUE']?></button>
                </div>
