...
if (!empty($arParams["FORM_NAME"])) {
    ?>
    <div class='main-title-text d-none'><?php echo $arParams["FORM_NAME"];?></div>
    <?php 
}
?>
...
<div class="slam-easyform<?=$arParams['HIDE_FORMVALIDATION_TEXT'] == 'Y' ? ' hide-formvalidation' : ''?>" <?=$WITH_FORM?>>
    <form id="<?=$FORM_ID?>"
        ...
        foreach($arResult['FORM_FIELDS'] as $fieldCode => $arField)
        {
            ...
            {
                ?>
                <input type="hidden" name="<?=$arField['NAME']?>" value="<?=$arField['VALUE'] ? $arField['VALUE'] : 'https://'.SITE_SERVER_NAME.$APPLICATION->GetCurPage(false);?>"/>
                <?
                    ...
                    if($arField['TYPE'] == 'textarea'):?>
                        <div class="col-xs-12">
                            <div class="form-group">
                                ...
                                <? endif; ?>
                                <div>
                                    <textarea class="form-control" id="<?=$arField['ID']?>" rows="5" name="<?=$arField['NAME']?>" placeholder="Задать вопрос" <?/*= $arParams["FORMATED_NAME"] ? 'Меня интересует оптовая цена на '.$arParams["FORMATED_NAME"] :  $arField['PLACEHOLDER_STR'];*/?> <?=$arField['REQ_STR']?> maxlength="600" style="height:70px;"><?= $arParams["FORMATED_NAME"] ? 'Меня интересует оптовая цена на '.$arParams["FORMATED_NAME"] :  $arField['VALUE'];?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-12">
                            <div class="form-group">
                                <? if(!$arParams['HIDE_FIELD_NAME']): ?>
                                    <label class="control-label" for="<?=$arField['ID']?>"><?=$arField['TITLE']?></label>
                                <? endif; 
                                if ($arField['ID'] == 'FORM3_FIELD_ИНН (для юридических лиц)') {
                                    $placeholder = "placeholder='ИНН (для юридических лиц)'";
                                } 
                                if ($arField['ID'] == 'FORM112_FIELD_Название компании'){
                                    $placeholder = "placeholder='Название компании'";
                                }
                                if ($arField['TITLE'] == 'Название компании'){
                                    $placeholder = "placeholder='Название компании'";
                                }
                                ?>
                                <input class="form-control" type="<?=$arField['TYPE'];?>" id="<?=$arField['ID']?>" name="<?=$arField['NAME']?>" value="<?=$arField['VALUE'];?>" <?/*if ($arField['ID'] == 'FORM3_FIELD_ИНН (для юридических лиц)') {echo 'placeholder'};*/?> <?=$arField['PLACEHOLDER_STR'];?> <?=$arField['REQ_STR']?> <?=$arField['MASK_STR']?> <?=$placeholder;?>>
                            </div>
                        </div>
                    <?endif;
                endforeach;?>
                <?if($arParams["USE_CAPTCHA"]):?>
                    <div class="col-xs-12">
                        <div class="form-group">
                            ...
                            <? endif; ?>
                            <input id="<?=$FORM_ID?>-captchaValidator"  class="form-control" type="text" required data-bv-notempty-message="<?=GetMessage("SLAM_REQUIRED_MESS");?>" name="captchaValidator" style="border: none; height: 0; padding: 0; visibility: hidden;padding:0px !important;min-height:0px !important;">
                            <div id="<?=$FORM_ID?>-captchaContainer"></div>
                        </div>
                    </div>
                <?endif;?>
                <div class="col-xs-12 text-left">
                <span style="font-size:14px;font-weight:700;color:#ffffff;">По вопросам обращайтесь на почту - <a href="mailto:belinin@traiv.ru">belinin@traiv.ru</a></span>
                </div>

                <div class="col-xs-12 pt-3 text-left">
                <?if($arResult['WARNING_MSG']):?>
                        <p class="warning-buy" style="color:#ffffff;"><!-- <small> --><?=$arResult['WARNING_MSG'];?><!-- </small> --></p>
                    <?endif;?>
                </div>
                
                <div class="col-xs-12 pt-3 pb-2 text-center">
                <button type="submit" class="btn-group-new btn-group-new-blue-land w-100" data-default="<?=$arParams['FORM_SUBMIT_VALUE']?>" style="border-radius:5px;height: 54px;font-size:16px;"><?=$arParams['FORM_SUBMIT_VALUE']?></button>
                </div>
