...
<div class="file-input file-extended">
	<ol style="position:absolute;height:0px;" class="file-placeholder-tbody webform-field-upload-list webform-field-upload-list-<?=$arParams["MULTIPLE"] == "Y" ? "multiple" : "single"?><?
	...
	<?if ($arParams["ALLOW_UPLOAD"] != "N")
	{
		?><div class="webform-field-upload" id="mfi-<?=$arParams['CONTROL_ID']?>-button"><?
			...
			?><span class="btn-group-new btn-group-new-gray webform-small-button webform-button-upload" style="overflow: hidden;
    width: 130px;
    height: 35px;
    line-height: 39px;
    cursor: pointer;
    ...
    margin: 0;"><?=$inputCaption?></span><?
		...
		if (!empty($arParams["ALLOW_UPLOAD_EXT"]) || $arParams['MAX_FILE_SIZE'] > 0)
		{
			...
			?>
            <div class="webform-field-upload-notice"><?=GetMessage("MFI_NOTICE_4")?></div>
