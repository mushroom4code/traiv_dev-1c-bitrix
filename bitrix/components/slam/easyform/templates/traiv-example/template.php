    ...
    <form id="<?=$FORM_ID?>"
        ...
        <div class="row">
            ...
            if(!empty($arResult['FORM_FIELDS'])):
                    ...
                    if($arField['TYPE'] == 'textarea'):?>
                        <div class="col-xs-12">
                            <div class="form-group">
                                ...
                                <? endif; ?>
                                <div>
                                    <textarea class="form-control" id="<?=$arField['ID']?>" rows="5" name="<?=$arField['NAME']?>" placeholder="Адрес для отправки образцов" <?/*= $arParams["FORMATED_NAME"] ? 'Меня интересует оптовая цена на '.$arParams["FORMATED_NAME"] :  $arField['PLACEHOLDER_STR'];*/?> <?=$arField['REQ_STR']?> maxlength="600" style="height:70px;"><?= $arParams["FORMATED_NAME"] ? 'Меня интересует оптовая цена на '.$arParams["FORMATED_NAME"] :  $arField['VALUE'];?></textarea>
                                </div>
                            </div>
                        </div>
                ...
                <span style="font-size:14px;font-weight:700;color:#ffffff;">Наши специалисты свяжутся с Вами в ближайшее время для уточнения параметров заявки.</span>
                </div>
