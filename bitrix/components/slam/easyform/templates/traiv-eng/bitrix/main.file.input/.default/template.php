...
<div class="file-input file-extended">
	...
	<?if ($arParams["ALLOW_UPLOAD"] != "N")
	{
		?><div class="webform-field-upload" id="mfi-<?=$arParams['CONTROL_ID']?>-button"><?
			...
			?><span class="btn btn-primary webform-small-button webform-button-upload">Select file<?/*=$inputCaption*/?></span><?
		...
		if (!empty($arParams["ALLOW_UPLOAD_EXT"]) || $arParams['MAX_FILE_SIZE'] > 0)
		{
			...
			?>
            <div class="webform-field-upload-notice"><?/*=GetMessage("MFI_NOTICE_4")*/?>Attach your Bank details, competitive invoice, or other files</div>
            <div class="webform-field-upload-notice">Select a file with an extension (doc, docx, xls, xlsx, txt, rtf, pdf, png, jpeg, jpg, gif) and a size that does not exceed 20 Mb.<?/*=str_replace(array("#ext#", "#size#"), array(htmlspecialcharsBx($arParams["ALLOW_UPLOAD_EXT"]), CFile::FormatSize($arParams['MAX_FILE_SIZE'])), $message);*/?></div><?
		}
	}
?>
