			...
			?>
				...
				<div class="kombox-combo kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
                    ...
                    <div class="kombox-filter-property-head">
                       ...
                       <?
                           /*$wrd = array('Длина', 'Диаметр', 'Материал', 'Система измерения крепежа');
                           $ico = array('L', '⌀', '<i class="fa fa-cube" aria-hidden="true"></i>', '<i class="fas fa-ruler-combined"></i>');
                       $arItem["NAME"] =  str_replace($wrd, $ico, $arItem["NAME"]);*/
                       $arItem["NAME"] =  str_replace('Система измерения крепежа', 'Система', $arItem["NAME"]);
                ...
                <i class="fa fa-angle-left" aria-hidden="true" style="cursor: pointer; display: none;"></i>
                    <div class="kombox-filter-property-elements">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						...
						<div class="lvl2<?echo $ar["DISABLED"]? ' kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>">
							...
							/>
                            <label for="<?echo $ar["CONTROL_ID"]?>" class="checkbox-label"><?echo $ar["VALUE"];?><!--span class="kombox-cnt">(<?//echo $ar["CNT"];?>)</span--></label>
						</div>
					<?endforeach;?>
                    </div>
                <i class="fa fa-angle-right" aria-hidden="true" style="cursor: pointer; display: none;"></i>
					<?komboxOtherValues($arItem);?>


                <?/* if ($arItem['CODE_ALT'] !== 'standart'):*/?><!--
                <div class="more" style="display: none; cursor: pointer;">Показать все</div><div class="less" style="display: none; cursor: pointer;">Скрыть</div></div>
            <?/* else:*/?>
                <div class="more standart" style="display: none; cursor: pointer;">Показать все</div><div class="less standart" style="display: none; cursor: pointer;">Скрыть</div></div>-->
