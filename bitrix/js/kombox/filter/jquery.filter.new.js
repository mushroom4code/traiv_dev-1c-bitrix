	...
	$.KomboxSmartFilter.prototype = {
		...
		reload: function(input)
		{
			...
			if(this.form.length)
			{
				...
				this.gatherInputsValues(values, this.form.find('input:not(.section-filter-ttl-search-input), select, .kombox-link .lvl2 a'));
		...
		postHandler: function (result)
		{
							...
							if(control.length)
							{
								...
								else
								{
									...
									if(ar.CNT > 0)
										...
										$('.modef-wrap a').animate({color: 'black'}, 2500, "linear", stop()).animate({color: 'white'}, 2500, "linear", stop())
									else
			...
			if ($('.serg_standart').length > 0) {
				$('.serg_standart').text('');
				$('#section-filter-block-606').find('label').not('.kombox-disabled').closest('.kombox-combo').each(function(e){
					$('.serg_standart').append($(this).attr('data-filter-val') + ", ");
				});
			}
		...
		gatherInputsValues: function (values, elements)
		{
			...
			if(elements.length)
			{
				elements.each(function(){
					...
					/*add DVK*/if (!el.hasClass('section-filter-ttl-search-input')) {
