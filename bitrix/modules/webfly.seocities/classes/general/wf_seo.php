...
class CWebflySeo {
     ...
     */
    public static function GetPageSeoTexts() {//Work OnEpilog?
        if (!strstr($_SERVER["REQUEST_URI"], '/bitrix/')) {
            ...
            if (is_array($arFieldsSeo) and $arFieldsSeo)
     ...
     */
    public static function ReplaceSeoTextsMacros(&$contents) {//Work OnEndBufferContent
        if (!strstr($_SERVER["REQUEST_URI"], '/bitrix/')) {
            $pageSeoTexts = self::$seo; //seo texts for current page
                if (is_array($pageSeoTexts) and $pageSeoTexts) {
