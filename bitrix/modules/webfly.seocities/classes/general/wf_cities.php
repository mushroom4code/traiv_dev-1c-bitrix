...
class CWebflyCities {
     ...
     * @return string
     */
    public static function GetBaseDomainOld() {
        $baseDomain = false;
     ...
     */
    public static function GetWFCitiesInfo() {//Work OnEpilog?
        if (!strstr($_SERVER["REQUEST_URI"], '/bitrix/')) {
            ...
            if (is_array($arFieldsCity) and $arFieldsCity)
     ...
     */
    public static function ReplaceRegionMacrosProcess(&$contents, $subdomain, $allCititesInfo) {
     ...
     * @param array $allCititesInfo - info about all elements from wf_citites IB
     */
    public static function ReplaceCurCityName(&$contents, $subdomain, $allCititesInfo) {
     ...
     * @param array $allCititesInfo - info about all elements from wf_citites IB
     */
    private static function ReplaceCurCityProps(&$contents, $subdomain, $allCititesInfo) {
            ...
            foreach ($allCititesInfo[$subdomain]['PROPS'] as $propCode => $propVal) {
                ...
                if (!is_array($propVal['VALUE'])) {
                    ...
                    if ($propVal['VALUE'] == '' or empty($propVal['VALUE'])) {
                        if (is_array($allCititesInfo['default']['PROPS'][$propCode]['VALUE']) && array_key_exists('TEXT', $allCititesInfo['default']['PROPS'][$propCode]['VALUE'])) {
     ...
     * @param array $allCititesInfo - info about all elements from wf_citites IB
     */
    public static function ReplaceAllCitiesPopup(&$contents, $subdomain, $allCititesInfo) {
...
<div class='region'>
     ...
     * @param string $contents - page buffered content
     */
    public static function ReplaceRegionMacros(&$contents) {//Work OnEndBufferContent
        if (!strstr($_SERVER["REQUEST_URI"], '/bitrix/')) {
            $subdomain = self::GetSubDomain();
            $allCititesInfo = self::$cities;
            if ($subdomain and ( is_array($allCititesInfo)) and $allCititesInfo)
