...
class CModuleOptions
{
	...
	private function SaveOptions()
	{
		...
		foreach( $this->arOptions as $opt => $arOptParams )
		{
			if($arOptParams['TYPE']!='CUSTOM')
			{
				...
				elseif(is_array( $val ))
					$val = serialize( $val );
				COption::SetOptionString( $this->module_id, $opt, $val, false, $siteId);
			}
			else
			{
				...
				if($pos!==false)
				{
					...
					if(is_array( $_POST[$opt] ))
					{
						...
						{
							COption::SetOptionString( $this->module_id, $opt, serialize( $ValArray ), false, $siteId );
						}
					}
				}
			}
	...
	private function GetCurOptionValues()
	{
		...
		foreach( $this->arOptions as $opt => $arOptParams )
		{
			if($arOptParams['TYPE']!='CUSTOM')
			{
				$this->arCurOptionValues[$opt] = COption::GetOptionString( $this->module_id, $opt, $arOptParams['DEFAULT'], $siteId );
