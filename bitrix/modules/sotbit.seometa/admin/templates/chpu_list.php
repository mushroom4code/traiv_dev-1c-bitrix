...
if($_REQUEST['set_filter'] != '' && CheckFilter())
{
    ...
    foreach ($FilterArr as $key) {
        if($_REQUEST[$key] != '') {
            $charset = mb_strtolower(mb_detect_encoding($_REQUEST[$key], array('utf-8', 'cp1251')));
...
{
    if(!in_array($field,array('ID','NAME','DATE_CHANGE','REAL_URL','NEW_URL', 'iblock_id', 'section_id', 'PRODUCT_COUNT', 'PROPERTIES', 'SITE_ID')))
    {
...
if(!in_array($by,array('ID','NAME','DATE_CHANGE','REAL_URL','NEW_URL', 'iblock_id', 'section_id', 'PRODUCT_COUNT', 'PROPERTIES', 'SITE_ID')))
{
    $by = 'ID';
}
...
while($arRes = $rsData->NavNext(true, ""))
{
    ...
    if($arRes['T']=='P')
    {
        ...
        $row->AddViewField("section_id", $section['NAME']);
