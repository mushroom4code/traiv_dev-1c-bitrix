...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid())
{
    ...
    if($_POST['SEOMETA_DATA']['ELEMENT_FILE'] && $_POST['SEOMETA_DATA']['ELEMENT_FILE_del'] == 'Y') {
        ...
        $_POST['SEOMETA_DATA']['ELEMENT_FILE']['tmp_name'] = $_SERVER['DOCUMENT_ROOT'] . '/upload/tmp' . $_POST['SEOMETA_DATA']['ELEMENT_FILE']['tmp_name'];
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        <textarea style="width: 69%" name="REAL_URL"><?php echo (isset( $chpu['REAL_URL'] ) && !empty( $chpu['REAL_URL'] )) ? $chpu['REAL_URL']:'';?></textarea>
    </td>
<tr>
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        <textarea style="width: 69%" name="NEW_URL"><?php echo (isset( $chpu['NEW_URL'] ) && !empty( $chpu['NEW_URL'] )) ? $chpu['NEW_URL']:'';?></textarea>
    </td>
<tr>
...
?>
    ...
    <?
        ...
        if(is_array($chpu['SITE_ID']))
            $arValue = $chpu['SITE_ID'];
        else if($chpu['SITE_ID'])
            $arValue = array($chpu['SITE_ID']);
        ...
        while($l_arr = $l->Fetch())
        {
            $s .=
                ...
                '<input type="checkbox" name="SITE_ID[]" value="'.htmlspecialcharsex($l_arr["LID"]).'" id="'.htmlspecialcharsex($l_arr["LID"]).'" class="typecheckbox"'.($arValue && $chpu['CONDITION_ID'] != 0 ? 'disabled ' : '') . ((in_array($l_arr["LID"], $arValue)?' checked':'')).'></div>'.
        ...
        if($arValue && $chpu['CONDITION_ID'] != 0) {
