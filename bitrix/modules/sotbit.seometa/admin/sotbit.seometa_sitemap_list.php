...
if(isset($_REQUEST['ID']) && !is_array($_REQUEST['ID'])) {
    $sitePaths = CSeoMetaSitemapLight::pathMainSitemap($_REQUEST['ID']);
}
...
if($arID = $lAdmin->GroupAction() && empty($errorMgs))
{
    if($_REQUEST['action'] == "delete")
    {
        foreach($_REQUEST['ID'] as $ID)
        {
            ...
            if(file_exists($sitePaths['abs_path'] . 'sitemap.xml'))
            {

                $xml = simplexml_load_file($sitePaths['abs_path'] . 'sitemap.xml');
                ...
                file_put_contents($sitePaths['abs_path'] . 'sitemap.xml', $xml->asXML());
            }
