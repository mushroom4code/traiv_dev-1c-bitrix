...
//    private $propertyIsset = false;
    ...
    public function getProperties() {
        ...
        if(isset($tagProperties['ProductProperty']) && !empty($tagProperties['ProductProperty'])) {
        ...
        if($this->SKU_IBLOCK_ID > 0
            && isset($tagProperties['OfferProperty'])
            && !empty($tagProperties['OfferProperty'])
        ) {
...
//    public function getSetedProperties($parsedRule) {
//        foreach ($parsedRule as $rule) {
//            foreach ($rule as $item) {
//                if($item->getMetaValue() && $item->getPropertyId()) {
//                    if(is_array($this->propertyIsset[$item->getPropertyId()])) {
//                        $this->propertyIsset[$item->getPropertyId()] = array_merge($this->propertyIsset[$item->getPropertyId()],
//                            [$item->getMetaValue()]);
//                    } else {
//                        $this->propertyIsset[$item->getPropertyId()] = [$item->getMetaValue()];
//                    }
//                }
//            }
//        }
//
//        if(is_array($this->propertyIsset)) {
//            foreach ($this->propertyIsset as &$arrProps) {
//                $arrProps = array_unique($arrProps);
//            }
//        }
//    }
    ...
    public function fillPropertyValues(PropertyCollection $propertyCollection, $sectionId) {
    ...
    private function elaborateHlPropertyValues(PropertyCollection $propertyCollection) {
    ...
    public function getPropertyValues($sectionId) {
        ...
        $rsElements = \CIBlockElement::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, false, array('ID' => $this->data[$this->IBLOCK_ID]));
        while($arElement = $rsElements->Fetch())
            $arElements[$arElement["IBLOCK_ELEMENT_ID"]] = $arElement;

        if (!empty($arElements) && $this->SKU_IBLOCK_ID &&  count((array) $this->data[$this->SKU_IBLOCK_ID]) > 0)
        {
            $arSkuFilter = array(
            ...
            $rsElements = \CIBlockElement::GetPropertyValues($this->SKU_IBLOCK_ID, $arSkuFilter, false, array('ID' => array_merge($this->data[$this->SKU_IBLOCK_ID], [$this->SKU_PROPERTY_ID])));
    ...
    private function fillItemValues($resultItem, $arProperty, $flag = null) {
...
//            return null;
        ...
        if($PROPERTY_TYPE == 'Ux') {
//            if(isset($this->propertyIsset[$PROPERTY_ID]) && !in_array($this->cache[$PROPERTY_ID][$key]['ID'], $this->propertyIsset[$PROPERTY_ID])) {
//                return null;
//            }
