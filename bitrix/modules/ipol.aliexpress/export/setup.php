...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (isset($arOldSetupVars['FILTER_PRODUCTS']))
		$filterProducts = $arOldSetupVars['FILTER_PRODUCTS'];
	if (isset($arOldSetupVars['FILTER_AVAILABLE']))
		$filterAvalable = $arOldSetupVars['FILTER_AVAILABLE'];
	if (isset($arOldSetupVars['FILTER_QUANTITY']))
		$filterQuantity = $arOldSetupVars['FILTER_QUANTITY'];
...
if ($STEP > 1)
{
	...
	if (!isset($USE_HTTPS) || $USE_HTTPS != 'Y')
		$USE_HTTPS = 'N';
	if (isset($_POST['FILTER_PRODUCTS']) && is_string($_POST['FILTER_PRODUCTS']))
		$filterProducts = $_POST['PRODUCTS'];
	...
	if (!isset($filterAvalable) || $filterAvalable != 'Y')
		$filterAvalable = 'N';
	if (isset($_POST['FILTER_QUANTITY']) && is_numeric($_POST['FILTER_QUANTITY']))
		$filterQuantity = $_POST['FILTER_QUANTITY'];
	if (!isset($filterQuantity) || !is_numeric($filterQuantity))
		$filterQuantity = 0;
...
$actionParams = "";
if ($adminSidePanelHelper && $adminSidePanelHelper->isSidePanel())
{
...
if ($STEP == 1)
{
	...
	if (!isset($SETUP_FILE_NAME))
		$SETUP_FILE_NAME = 'aliexpress_'.mt_rand(0, 999999).'.php';
...
<?php if ($ACTION=="EXPORT_SETUP" || $ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') { ?>
	<tr>
		<td width="40%">
			<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_PROFILE_NAME') ?>" alt="">
...
<tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_IBLOCK') ?>" alt="">
		<?=GetMessage('BX_CATALOG_EXPORT_IBLOCK'); ?>
	</td>
	<td width="60%"><?
...
<tr id="tr_SITE_ID" style="display: <?=(count($currentList) > 1 ? 'table-row' : 'none' ); ?>;">
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_YANDEX_SITE') ?>" alt="">
		<?=GetMessage('BX_CATALOG_EXPORT_YANDEX_SITE'); ?>
	</td>
	<td width="60%">
		<script type="text/javascript">
...
<tr>
	<td width="40%" valign="top">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_GROUP') ?>" alt="">
...
<tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_PRODUCTS') ?>" alt="">
		<?= GetMessage('CAT_YANDEX_FILTER_PRODUCTS') ?>
	</td>
	<td width="60%">
		<textarea name="FILTER_PRODUCTS" cols="50" rows="5"><?= $filterProducts ?></textarea>
		<br>
		<small><?= GetMessage('CAT_YANDEX_FILTER_PRODUCTS_HELP') ?></small>
	</td>
</tr>
		...
		function showDetailPopup()
		{
			if (!obDetailWindow)
			{
				...
				var obDetailWindow = new BX.CAdminDialog({
					'content_url': '/bitrix/tools/ipol.aliexpress/export_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value,
...
<tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_AVAILABLE') ?>" alt="">
		<? echo GetMessage('CAT_YANDEX_FILTER_AVAILABLE'); ?>
	</td>
	<td width="60%">
...
<tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_QUANTITY') ?>" alt="">
		<? echo GetMessage('CAT_YANDEX_FILTER_QUANTITY') ?>
	</td>
	<td width="60">
		<input type="text" name="FILTER_QUANTITY" value="<?= $filterQuantity ?: 0 ?>">
	</td>
</tr>
...
?><tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_MAX_EXECUTION_TIME') ?>" alt="">
		<?=GetMessage('CAT_MAX_EXECUTION_TIME');?>
	</td>
	<td width="60%">
...
<tr>
	<td width="40%">
		<img src="/bitrix/js/main/core/images/hint.gif" title="<?= GetMessage('IPOL_ALI_EXPORT_USE_HTTPS') ?>" alt="">
		<? echo GetMessage('CAT_YANDEX_USE_HTTPS'); ?>
	</td>
	<td width="60%">
...
if (2 > $STEP)
{
	...
	<input type="hidden" name="SETUP_FIELDS_LIST" value="V,IBLOCK_ID,SITE_ID,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_PRODUCTS,FILTER_AVAILABLE,FILTER_QUANTITY,DISABLE_REFERERS,MAX_EXECUTION_TIME,CHECK_PERMISSIONS">
