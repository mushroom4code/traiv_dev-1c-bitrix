	...
	if (!empty($_REQUEST['save']))
	{
		...
		if (empty($arErrors))
		{
			$arXMLData = array(
				...
				'DISABLE_PRICE_DISCOUNT' => intval($_POST['DISABLE_PRICE_DISCOUNT']),
...
?>
		...
		<tr class="heading">
			<td colspan="2">
				<?=GetMessage('BX_CATALOG_EXPORT_YANDEX_COMMON_FIELDS1')?>
			</td>
		</tr>
					...
					<div id="config_<?=htmlspecialcharsbx($key)?>" style="padding: 10px; display: <? echo ($type == $key ? 'block' : 'none'); ?>;">
						<table width="90%" class="inner" style="text-align: center;">
							<? foreach ($arConfig as $headerText => $prop): ?>
								<?if ($headerText === 'YANDEX_PROP1_header_sku') { ?>
									<tr>
										<td align="right"><?= GetMessage('YANDEX_DIMENSSIONS_LENGTH') ?></td>
										<td>
											<select name="COMMON_FIELDS[DIMENSSIONS_MM]">
												<option value="1"    <?= $commonFields['DIMENSSIONS_MM'] == 1 ? 'selected' : '' ?>><?= GetMessage('YANDEX_DIMENSSIONS_LENGTH_MM') ?></option>
												<option value="10"   <?= $commonFields['DIMENSSIONS_MM'] == 10 ? 'selected' : '' ?>><?= GetMessage('YANDEX_DIMENSSIONS_LENGTH_CM') ?></option>
												<option value="1000" <?= $commonFields['DIMENSSIONS_MM'] == 1000 ? 'selected' : '' ?>><?= GetMessage('YANDEX_DIMENSSIONS_LENGTH_M') ?></option>
											</select>
										</td>
									</tr>

									<tr>
										<td align="right"><?= GetMessage('YANDEX_DIMENSSIONS_WEIGHT') ?></td>
										<td>
											<select name="COMMON_FIELDS[DIMENSSIONS_WEIGHT]">
												<option value="1"    <?= $commonFields['DIMENSSIONS_WEIGHT'] == 10 ? 'selected' : '' ?>><?= GetMessage('YANDEX_DIMENSSIONS_WEIGHT_G') ?></option>
												<option value="1000" <?= $commonFields['DIMENSSIONS_WEIGHT'] == 1000 ? 'selected' : '' ?>><?= GetMessage('YANDEX_DIMENSSIONS_WEIGHT_KG') ?></option>
											</select>
										</td>
									</tr>
...
function __addYP()
{
	...
	oCell.innerHTML = strContent;
			...
			<tr>
				<td valign="top"><?= GetMessage('YANDEX_STORE_SELECT') ?></td>
				<td>
					<?php
						$items = \Bitrix\Catalog\StoreTable::getList([
							'filter' => ['ACTIVE' => true],
							'select' => ['ID', 'TITLE'],
					...
					<select name="COMMON_FIELDS[STORE][]" id="" multiple="multiple">
						<option value="ALL"><?= GetMessage('YANDEX_STORE_SELECT_ALL') ?></option>

						<? foreach($items as $item) { ?>
							<option value="<?= $item['ID'] ?>" <?= in_array($item['ID'], $commonFields['STORE']) ? 'selected' : '' ?> ><?= $item['TITLE'] ?></option>
						<? } ?>
					</select>
				</td>
			</tr>
...
?>
	...
	<tr>
		<td width="50%"><?=GetMessage('YANDEX_DISABLE_PRICE_DISCOUNT')?></td>
		<td><input type="checkbox" name="COMMON_FIELDS[DISABLE_PRICE_DISCOUNT]" value="1" <?= $commonFields['DISABLE_PRICE_DISCOUNT'] ? 'checked' : '' ?> /></td>
	</tr>
