...
else
{
	...
	if (!empty($ARTICUL_CODE)) {
		$prop = \CIBlockProperty::GetList($sort = [], $filter = ['IBLOCK_ID' => $IBLOCK_ID, 'CODE' => $ARTICUL_CODE])->fetch();
		if ($prop) $IBLOCK_ARTICUL_ID = $prop['ID'];
	}
	...
	if (!empty($arOffers['IBLOCK_ID']))
	{
		...
		if (!empty($ARTICUL_CODE)) {
			$prop = \CIBlockProperty::GetList($sort = [], $filter = ['IBLOCK_ID' => $intOfferIBlockID, 'CODE' => $ARTICUL_CODE])->fetch();
			if ($prop) $OFFERS_ARTICUL_ID = $prop['ID'];
		}
...
if (empty($arRunErrors))
{
	...
	do
	{
		...
		if (!empty($commonFields['STORE']) && !in_array('ALL', $commonFields['STORE'])) {
			$commonStoreFilter = ['LOGIC' => 'OR', ['CATALOG_TYPE' => \Bitrix\Catalog\ProductTable::TYPE_SKU]];
			$offersStoreFilter = ['LOGIC' => 'OR'];

			foreach($commonFields['STORE'] as $storeId) {
				$itemFields[]  = 'CATALOG_STORE_AMOUNT_'. $storeId;
				$offerFields[] = 'CATALOG_STORE_AMOUNT_'. $storeId;

				$commonStoreFilter[] = ['!=CATALOG_STORE_AMOUNT_'. $storeId => false];
				$offersStoreFilter[] = ['!=CATALOG_STORE_AMOUNT_'. $storeId => false];
			}

			$filter       = array_merge($filter, [$commonStoreFilter]);
			$offersFilter = array_merge($offersFilter, [$offersStoreFilter]);
		}
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						$articul       = $OFFERS_ARTICUL_ID ? $offer['PROPERTIES'][ $OFFERS_ARTICUL_ID ]['VALUE'] : $offer['ID'];
						$articul       = \Bitrix\Main\Text\Encoding::convertEncoding($articul ?: $offer['ID'], SITE_CHARSET, 'cp1251');

						$articulParent = $IBLOCK_ARTICUL_ID ? $row['PROPERTIES'][ $IBLOCK_ARTICUL_ID ]['VALUE'] : $row['ID'];
						$articulParent = \Bitrix\Main\Text\Encoding::convertEncoding($articulParent ?: $row['ID'], SITE_CHARSET, 'cp1251');
						
						$available = ' available="'.($offer['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
						$itemsContent .= '<offer id="'.$articul.'"'.$productFormat.$available .' group_id="'. $articulParent .'"' .">\n";
						$itemsContent .= '<barcode>'. $articul .'</barcode>';
						...
						if (!empty($picture))
							$itemsContent .= "<sku_picture>".$picture."</sku_picture>\n";
						unset($picture);
								...
								case 'quantity':
									...
									if ($key == 'quantity') {
										if (!empty($commonFields['STORE']) && !in_array('ALL', $commonFields['STORE'])) {
											$value = 0;
											foreach($commonFields['STORE'] as $storeId) {
												$value += $offer['CATALOG_STORE_AMOUNT_'. $storeId];
											}
										}

										if ($filterQuantity > $value || $offer['CATALOG_AVAILABLE'] != 'Y') {
											$value = 0;
										}
									}
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					$articul = $IBLOCK_ARTICUL_ID ? $row['PROPERTIES'][ $IBLOCK_ARTICUL_ID ]['VALUE'] : $row['ID'];
					$articul = \Bitrix\Main\Text\Encoding::convertEncoding($articul ?: $row['ID'], SITE_CHARSET, 'cp1251');

					$available = ' available="'.($row['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
                    $itemsContent .= '<offer id="'.$articul.'"'.$productFormat.$available .' group_id="'. $articul .'">'."\n";
					$itemsContent .= '<barcode>'. $articul .'</barcode>';
					unset($available);
							...
							case 'quantity':
								...
								if ($key == 'quantity') {
									if (!empty($commonFields['STORE']) && !in_array('ALL', $commonFields['STORE'])) {
										$value = 0;
										foreach($commonFields['STORE'] as $storeId) {
											$value += ($row['CATALOG_STORE_AMOUNT_'. $storeId] ?: 0);
										}
									}

									if ($filterQuantity > $value || $row['CATALOG_AVAILABLE'] != 'Y') {
										$value = 0;
									}
								}
