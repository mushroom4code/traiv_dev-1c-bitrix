...
$addPath = ($request->get('url_data_file')) ? $request->get('url_data_file') : '/';
...
if ($request->isPost() && $action == 'optimize') {
    ...
    echo \AdwMinified\Image::find($request->get('limit'), $request->get('offset'), $addPath);
...
if ($request->isPost() && $action == 'getcount') {
    ...
    echo \AdwMinified\Image::getCount($addPath, false, $dirignore);
...
if ($request->isPost() && $action == 'save_position') {
    ...
    echo \AdwMinified\Image::savePosition($request->get('directory'), $request->get('position'), 0);
...
if ($request->isPost() && $action == 'reset_position') {
    ...
    echo \AdwMinified\Image::savePosition('', 0, 0);
...
<div class="adwex-optiimg">
    ...
    $saved_position = \AdwMinified\Image::getPosition('', 0 , 0);
    ...
    <tr>
        ...
        <td>
            <div id="optiimg_compress_alltime_div">
                <?
                echo Loc::getMessage("ADWMINI_OPTIMIZE_ALLTIME_SAVED") , ': '; ?>
                <span id="alltime_economy"></span>
            </div>
        </td>
    </tr>
    <tr class="heading">
...
<script>
                ...
                function(result){
                    ...
                    if (parseInt(files_ready) < parseInt(f_count) && parseInt(result.count) != 0 && !result.stop){
                        ...
                        result.errors.forEach(function(entry) {
                            $('.message-log-wrap ul').append('<li class="error"><span class="filename">' + entry.file + '</span><span class="filestatus">' + entry.response + '</span></li>');
                        });
                        ...
                        running = document.getElementById('start_button').disabled = false;
                        result.errors.forEach(function(entry) {
                            document.getElementById('optiimg_compress_result_div').innerHTML = '<div class="adm-info-message-wrap adm-info-message-red"><div class="adm-info-message"><div class="adm-info-message-title"><?echo Loc::getMessage("ADWMINI_OPTIMIZE_ERROR")?> ' + entry.response + '</div><div class="adm-info-message-icon"></div></div></div>';
                        });
                    } else {
                        CloseWaitWindow();
                        running = document.getElementById('start_button').disabled = false;
    ...
    function showEconomy (bytes) {
        bytes = parseInt(bytes);
        window.alltime_economy.innerHTML = formatSize(bytes);
    }
    
    showEconomy(<?= \COption::GetOptionString(\AdwMinified\Tools::moduleID, 'WAS_SAVED_IMAGE', 0) ?>);
