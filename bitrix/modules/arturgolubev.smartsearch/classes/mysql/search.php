		...
		if (strpos($strSort, "TITLE_RANK") !== false)
		{
			...
			if ($bStem)
			{
				foreach ($this->Query->m_stemmed_words as $stem)
				{
					if (strlen($strSelect) > 0)
						$strSelect .= " + ";
					$strSelect .= "if(locate('".$stem."', upper(sc.TITLE)) > 0, locate('".$stem."', upper(sc.TITLE)), 250)";
				}
				$arSelect["TITLE_RANK"] = $strSelect." as TITLE_RANK";
			}
			else
			{
				foreach ($this->Query->m_words as $word)
				{
					if (strlen($strSelect) > 0)
						$strSelect .= " + ";
					$strSelect .= "if(locate('".$DB->ForSql(ToUpper($word))."', upper(sc.TITLE)) > 0, locate('".$DB->ForSql(ToUpper($word))."', upper(sc.TITLE)), 250)";
				}
				$arSelect["TITLE_RANK"] = $strSelect." as TITLE_RANK";
			}
