...
class CKomboxCatalogFilter extends CBitrixComponent
{
		...
		foreach(CIBlockSectionPropertyLink::GetArray($IBLOCK_ID, $this->SECTION_ID) as $PID => $arLink)
		{
			...
			if($arProperty)
			{
				...
				$items[$arProperty['ID']] = array(
					...
					'VALUES' => array(),
					'CLOSED' => $arLink["DISPLAY_EXPANDED"] == 'Y' ? false : true
				);
			}
		}
	...
	public function getResultItems()
	{
		...
		foreach($items as &$arItem){
			if(in_array($arItem['CODE'], $this->arParams['CLOSED_PROPERTY_CODE']) && strlen($arItem['CODE']))
				$arItem['CLOSED'] = true;
			elseif(isset($this->arParams['CLOSED_PROPERTY_CODE'])){
				$arItem['CLOSED'] = false;
			}
		...
		if($this->SKU_IBLOCK_ID)
		{
			...
			foreach($this->getIBlockItems($this->SKU_IBLOCK_ID) as $PID => $arItem)
			{
				if(in_array($arItem['CODE'], $this->arParams['CLOSED_OFFERS_PROPERTY_CODE']) && strlen($arItem['CODE']))
					$arItem['CLOSED'] = true;
				elseif(isset($this->arParams['CLOSED_OFFERS_PROPERTY_CODE']))
					$arItem['CLOSED'] = false;
	...
	protected function getRequestBySef()
	{
		...
		if(CModule::IncludeModule('kombox.filter'))
		{
			...
			if(!empty($arUnavailableParams)){
				if(!defined('KOMBOX_FILTER_ERROR_404'))
					@define('KOMBOX_FILTER_ERROR_404', 'Y');
				CKomboxFilter::Set404(true);
				CKomboxFilter::SetUnavailableParams($arUnavailableParams);
			}
		}
		...
		foreach($this->arResult['ELEMENTS'] as &$res)
		{
			...
			if(is_array($this->arResult['ELEMENTS_SKU'][$id]))
			{
				...
				foreach($this->arResult['ELEMENTS_SKU'][$id] as $k => &$resSku)
				{
					...
					if($this->checkElementByHash($allCHECKED, $mergeHash))
					{
						$filterIDs[$id] = $id;
					}
				}
	...
	public function getUrl($url, $arRequest = array(), $addParams = array())
	{
		...
		if($this->arResult['IS_SEF']){
			//учтем, что $url могут быть get-параметры
			$arUrl = \CHTTP::ParseURL($url);
			$sefURL = $arUrl['path'];

			if(!empty($arUrl['query'])){
				foreach(explode('&', $arUrl['query']) as $param){
					$arParam = explode('=', $param);
					if(!empty($arParam[0])){
						$addParams[$arParam[0]] = $arParam[1];
					}
				}
			}
		}
	...
	protected function getReturn()
	{
		...
		$seo = new Kombox\Filter\Seo($this->arResult +  array(
