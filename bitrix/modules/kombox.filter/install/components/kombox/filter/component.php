...
if($this->StartResultCache(false, array($arrFilter,($arParams['CACHE_GROUPS']? $USER->GetGroups(): false), $arResult['IS_SEF'])))
{
	...
	if(!empty($arResult['ITEMS']))
	{
		...
		$res = CIBlockElement::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, false, array('ID' => $this->arResult["PROPERTY_ID_LIST"]));
		...
		if(!empty($arIDs) && $this->SKU_IBLOCK_ID)
		{
			...
			$res = CIBlockElement::GetPropertyValues($this->SKU_IBLOCK_ID, $arSkuFilter, false, array('ID' => $this->arResult["SKU_PROPERTY_ID_LIST"]));
			while($el = $res->Fetch())
			{
...
)
{
	...
	$GLOBALS[$arParams["PAGER_PARAMS_NAME"]]["BASE_LINK"] = $this->getUrl($pageURL, $arResult['REQUEST']);
}

$arResult['FORM_ACTION'] = $this->getUrl($pageURL);
$arResult['HIDDEN'] = $this->getHidden();
$arResult['FILTER_URL'] = $this->getUrl($pageURL, $arResult['REQUEST']);
$arResult['FILTER_AJAX_URL'] = $this->getUrl($pageURL, $arResult['REQUEST']);

if (isset($_GET['bxajaxid']))
{
	if(is_array($_GET['bxajaxid']))
		$arResult['FILTER_AJAX_URL'] = CHTTP::urlAddParams($arResult['FILTER_AJAX_URL'], array('bxajaxid' => $_GET['bxajaxid'][0]));
	else
		$arResult['FILTER_AJAX_URL'] = CHTTP::urlAddParams($arResult['FILTER_AJAX_URL'], array('bxajaxid' => $_GET['bxajaxid']));
	
	?>
	<script type="text/javascript">
		top.BX.ajax.history.put(window.AJAX_PAGE_STATE.getState(), '<?=$arResult['FILTER_URL']?>');
	</script>
	<?
}
