...
CModule::AddAutoloadClasses(
	...
	array(
		...
		"\\Kombox\\Filter\\PropertySettingsTable" => "lib/property_settings.php",
		"Kombox\\Filter\\SeoTable" => "lib/seo.php",
		"\\Kombox\\Filter\\SeoTable" => "lib/seo.php",
		"Kombox\\Filter\\SeoPropertiesTable" => "lib/seo_properties.php",
		"\\Kombox\\Filter\\SeoPropertiesTable" => "lib/seo_properties.php",
		"Kombox\\Filter\\Tools" => "classes/general/tools.php",
		"\\Kombox\\Filter\\Tools" => "classes/general/tools.php",
		"Kombox\\Filter\\Seo" => "classes/general/seo.php",
		"\\Kombox\\Filter\\Seo" => "classes/general/seo.php",
	)
);
...
class CKomboxFilter
{
	...
	private static $bError404 = false;
	private static $arUnavailableParams = array();
	...
	public static function SetCurPage($url)
	{
		self::$sDocPath2 = $url;
	}
	
	public static function Set404($b404 = true)
	{
		self::$bError404 = $b404;
	}
	...
	{
		return self::$bError404;
	}
	
	public static function GetUnavailableParams()
	{
		if(self::$bError404)
			return self::$arUnavailableParams;
	}
	
	public static function SetUnavailableParams($ar = array())
	{
		if(is_array($ar))
			self::$arUnavailableParams = $ar;
		else
			self::$arUnavailableParams = array();
	}
	...
	private static function addDialogFilterParams()
	{
    ...
    { 
		...
		if(!isset($arConvertParams[$key]))
		{
			...
			if (strlen($valDate)<=0) 
				$valDate = date("Y-m-d"); 
			list($dpYear, $dpMonth, $dpDay) = explode("-", $valDate);
			$dpDay += 1; 
	...
	public function OnBuildGlobalMenu(&$aGlobalMenu, &$aModuleMenu) 
	{
		...
		{
			if(\Bitrix\Main\ModuleManager::isModuleInstalled('seo') && Loader::includeModule('iblock'))
			{
				...
				if(count($iblocks_seo))
				{
					$set = false;
					foreach($aModuleMenu as &$menu){
						if($menu['section'] == 'seo'){
							$item = array(
								'text' => Loc::GetMessage('KOMBOX_MODULE_FILTER_SEO_MENU_ITEM'),
								'items_id' => 'kombox_filter_seo',
								'items' => array()
							);
							...
							{
								if(in_array($arIBlock['ID'], $iblocks_seo))
								{
									$item['items'][] = array(
										'text' => $arIBlock['NAME'],
										'url' => 'kombox_filter_seo.php?lang='.LANGUAGE_ID.'&iblock_id='.$arIBlock['ID']
									);
								}
							}
...
class CKomboxFilterIBlock extends CIBlockElement
{
	public static function GetPropertyValues($IBLOCK_ID, $arElementFilter, $arProperties = array(), $extMode = false)
    {
        ...
        $element->strField = "ID";
        $element->prepareSql(array(), $arElementFilter, false, false, array("ID"));
